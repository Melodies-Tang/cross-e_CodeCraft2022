...
def greedyAssign(valueMat, chances):
    ...
    for time in range(T):
        current_sites = sorted(enumerate(valueMat[time]), key=sortByPotential).__iter__()
...
def reassign(N, T, time, add_to_site_idx, bandwidth, site_name, site_client, client_site_band, demands, site_time_used, POS_95):
    # reassign
    ...
    for i, client in enumerate(site_client[add_to_site_name]):
        ...
        current_assign = client_site_band[client].get(add_to_site_name, 0)  # to add_to_site
        add = min(client_avg_add, demands[time][client] - current_assign)
        client_site_band[client][add_to_site_name] = current_assign + add
        site_time_used[add_to_site_name][time] += add
        potential -= add
        while add > 0:
            relative_site_cnt = len(client_site_band[client]) - 1
            j = 0
            del_site = []
            for site, band in client_site_band[client].items():
                if site != add_to_site_name:
                    ...
                    client_site_band[client][site] -= minus
                    if client_site_band[client][site] == 0:
                        del_site.append(site)
                    site_time_used[site][time] -= minus
                    add -= minus
                    j += 1
            for site in del_site:
                client_site_band[client].pop(site)
...
def evaluate(solution, demands, site_bandwidth, site_client, client_site):
    ...
    evaluation = [[0 for i in range(N)] for j in range(T)]
    ...
    for site_idx in range(N):
        ...
        for time in range(T):
            ...
            evaluation[time][site_idx] = reassign(N, T, time, site_idx, site_bandwidth[site], site_name, site_client, copy.deepcopy(time_client_site_band[time]), demands, copy.deepcopy(site_time_used), POS_95)
    ...
    for time in range(T):
        ...
        potential = site_bandwidth[add_to_site_name] - site_time_used[add_to_site_name][time]
